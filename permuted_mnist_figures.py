...
LOG_TO_WANDB = True
wandb_project = 'streaming_permuted_mnist_figures'
wandb_user = ''
...
def run_training():
    if len(sys.argv) > 1:        
        ...
        # config['output_act'] = 'gelu'
        config['learning_rate'] = 6e-4

        if LOG_TO_WANDB:
            # wandb.init(project=wandb_project, entity=wandb_user, config = config, name = 'gelu')
            # wandb.init(project=wandb_project, entity=wandb_user, config = config, name = 'gelu + sgd')
            # wandb.init(project=wandb_project, entity=wandb_user, config = config, name = 'sgd')
            # wandb.init(project=wandb_project, entity=wandb_user, config = config, name = 'pairwise lower lr')
            wandb.init(project=wandb_project, entity=wandb_user, config = config, name = 'pairwise 6e-4 lr')
    ...
    for i_run in range(hyperparams['num_runs']):
        ...
        if hyperparams['split_experiment']:
            ...
            for task, permutation in enumerate(tqdm(permutations)):
                ...
                def p_log():
                    test_accuracies = []
                    for permutation in permutations[:task+1]:
                    ...
                    for i in range(task+1):
                        l[f'Taks {i} error'] = 1.- test_accuracies[i]
