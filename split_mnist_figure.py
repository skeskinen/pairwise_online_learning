...
LOG_TO_WANDB = True
wandb_project = 'split_mnist_percentages_figure'
wandb_user = ''
...
def run_training():
    if len(sys.argv) > 1:
        ...
        if LOG_TO_WANDB:
            wandb.init(project=wandb_project, entity=wandb_user, config = config, name = 'pairwise')

        config['num_runs'] = 1

        for p in range(70):
            config['wta_p'] = 0.01 + p * 0.01
            train_with_hyperparams(config)
        return
...
def train_with_hyperparams(hyperparams, do_logging=True):
    ...
    for i_run in range(hyperparams['num_runs']):
        ...
        if hyperparams['split_experiment']:
            ...
            if do_logging and i_run == 0:
                log_dict = {'accuracy': accuracy, 'visit': visit, 'split': i_split, 'epoch': epoch, 'wta_p': hyperparams['wta_p']}
